    ...
    def download_all_panopto_streams(
        ...
        if include_composed:
            ...
            if self._is_complete(composed_path):
                ...
                # Download to local temp then move to NAS to avoid SMB close()/flush()
                # failures on large composed files. Use a temp DIRECTORY so the target
                # filename doesn't pre-exist (yt-dlp would skip an empty pre-existing file).
                import tempfile, shutil
                try:
                    with tempfile.TemporaryDirectory(prefix="panopto_") as _tmp_dir:
                        tmp_path = Path(_tmp_dir) / "composed.mp4"
                        cmd = self._yt_dlp_stream_cmd(tmp_path, cdn_url)
                        proc = subprocess.run(cmd, capture_output=False, check=False)
                        if proc.returncode == 0 and tmp_path.exists() and tmp_path.stat().st_size >= 1_000_000:
                            console.print(f"  [dim]Moving to NAS: {format_bytes(tmp_path.stat().st_size)}…[/dim]")
                            shutil.move(str(tmp_path), str(composed_path))
                ...
                if cookies_file:
                    ...
                    if part_path.exists():
                        ...
                        part_path.unlink(missing_ok=True)
                    # Download to a local temp DIRECTORY first, then move to the NAS.
                    # SMB writes intermittently fail on close()/flush() with [Errno 22]
                    # even when all bytes were written. Using a temp directory ensures
                    # the target filename doesn't pre-exist (would otherwise trick
                    # yt-dlp into thinking the file is "already downloaded").
                    import tempfile, shutil
                    try:
                        with tempfile.TemporaryDirectory(prefix="panopto_") as _tmp_dir:
                            tmp_path = Path(_tmp_dir) / "composed.mp4"
